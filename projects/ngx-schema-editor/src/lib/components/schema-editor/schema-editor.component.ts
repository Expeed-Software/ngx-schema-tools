...
import { AllowedValue, isLabeledValue, parseOneOf, toOneOf, hasLabeledValues } from './models/allowed-value.model';
...
export class SchemaEditorComponent {
  ...
  private hasUncommittedChildFields(fields: EditorField[]): boolean {
    for (const field of fields) {
      if (field.children) {
        if (field.children.some(c => c.isEditingDefault || c.isEditingValues || c.isEditingValidators || !c.name || c.expanded)) {
          return true;
        }
  ...
  // Handle add field after from FieldItemComponent
  onFieldAddAfter(field: EditorField): void {
    const index = this.fields().indexOf(field);
    if (index > -1) {
      const newField: EditorField = {
        id: this.generateId(),
  ...
  onFieldTypeChange(field: EditorField, type: string): void {
    ...
    if (previousType !== type && field.allowedValues?.length) {
  ...
  private jsonSchemaPropertyToEditorField(name: string, schema: JsonSchema, isRequired: boolean): EditorField {
    ...
    const field: EditorField = {
      ...
      label: schema.title,
      required: isRequired,
      allowedValues: this.parseAllowedValues(schema),
    ...
    if (schema.type === 'object' && schema.properties) {
      ...
      if (typeof schema.items !== 'boolean' && !Array.isArray(schema.items)) {
        // Determine item type from schema.items
        const itemType = this.jsonSchemaTypeToEditorType(schema.items);
        field.itemType = itemType as EditorField['itemType'];
  ...
  // Parse allowed values from JSON Schema (handles both enum and oneOf formats)
  private parseAllowedValues(schema: JsonSchema): AllowedValue[] | undefined {
    // Handle simple enum
    if (schema.enum && Array.isArray(schema.enum)) {
      return schema.enum as AllowedValue[];
    }

    // Handle oneOf with const/title (labeled values)
    if (schema.oneOf && Array.isArray(schema.oneOf)) {
      const values = parseOneOf(schema.oneOf);
  ...
  private fieldToJsonSchema(field: EditorField): object {
    ...
    if (field.type === 'date') {
      ...
      if (itemType === 'object') {
        // Object items - use children as properties
        ...
        schema['items'] = { type: 'string', format: 'date-time' };
    ...
    if (field.allowedValues && field.allowedValues.length > 0 && field.type !== 'array') {
      if (hasLabeledValues(field.allowedValues)) {
        ...
        // Use simple enum for primitive values
