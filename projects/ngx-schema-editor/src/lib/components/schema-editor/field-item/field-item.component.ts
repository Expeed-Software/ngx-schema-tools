...
import { AllowedValue, LabeledValue, isLabeledValue, getRawValue, getDisplayText } from '../models/allowed-value.model';

// Re-export for consumers
export type { AllowedValue, LabeledValue } from '../models/allowed-value.model';
...
export class FieldItemComponent implements OnInit, OnChanges {
  ...
  @Output() addAfter = new EventEmitter<EditorField>();
  ...
  // Item types for arrays (excludes 'array' since we don't support nested arrays)
  arrayItemTypes: Array<{ value: string; label: string; icon: string }> = [
    { value: 'string', label: 'String', icon: 'text_fields' },
    { value: 'number', label: 'Number', icon: 'pin' },
    { value: 'boolean', label: 'Boolean', icon: 'toggle_on' },
    { value: 'date', label: 'Date', icon: 'calendar_today' },
  ...
  // Whether new allowed value should be labeled (with title)
  newValueLabeled = false;

  // Use shared helper functions from allowed-value.model
  isLabeledValue = isLabeledValue;
  ...
  onFieldTypeChange(type: string): void {
    const previousType = this.field.type;
    this.field.type = type as EditorField['type'];
    if (type === 'object' && !this.field.children) {
      this.field.children = [];
    }
    if (type === 'array') {
      // Default item type to string if not set
      if (!this.field.itemType) {
        this.field.itemType = 'string';
      }
      // Initialize children only if item type is object
      if (this.field.itemType === 'object' && !this.field.children) {
        this.field.children = [];
      }
    }
    ...
    if (previousType !== type && this.field.allowedValues?.length) {
  ...
  onItemTypeChange(itemType: string): void {
    this.field.itemType = itemType as EditorField['itemType'];
    if (itemType === 'object') {
      // Initialize children for object item type
      if (!this.field.children) {
        this.field.children = [];
      }
    } else {
  ...
  onChildAddAfter(child: EditorField): void {
    if (this.field.children) {
      const index = this.field.children.indexOf(child);
      if (index > -1) {
        const newField: EditorField = {
        ...
        this.field.children.splice(index + 1, 0, newField);
        this.fieldChange.emit();
      }
    }
  }
  ...
  addAllowedValue(valueInput: HTMLInputElement, labelInput?: HTMLInputElement): void {
    const rawValue = valueInput.value.trim();
    if (!rawValue) return;

    let newValue: AllowedValue;
    ...
    // Create labeled or simple value
    if (this.newValueLabeled && labelInput) {
      const title = labelInput.value.trim();
      if (!title) return;
      newValue = { const: parsedValue, title };
    } else {
      newValue = parsedValue;
    }

    // Check for duplicates based on raw value
    const newRawValue = this.getRawValue(newValue);
    const isDuplicate = this.field.allowedValues?.some(v => this.getRawValue(v) === newRawValue);
  ...
  onAllowedValueKeydown(event: KeyboardEvent, valueInput: HTMLInputElement, labelInput: HTMLInputElement): void {
    if (event.key === 'Enter') {
      event.preventDefault();
      this.addAllowedValue(valueInput, labelInput);
    }
  }
