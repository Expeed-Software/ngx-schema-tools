...
import { SchemaTreeNode, FieldMapping, DefaultValue } from '../models/schema.model';
...
export class SchemaTreeComponent implements AfterViewInit, OnDestroy {
  @Input() schema!: { name: string; fields: SchemaTreeNode[] };
  ...
  toggleExpand(field: SchemaTreeNode, event: Event): void {
    event.stopPropagation();
    field.expanded = !field.expanded;
    setTimeout(() => this.emitFieldPositions(), 50);
  }

  onDragStart(event: MouseEvent, field: SchemaTreeNode): void {
  ...
  onDrop(event: MouseEvent, field: SchemaTreeNode): void {
    const element = event.currentTarget as HTMLElement;
    const rect = element.getBoundingClientRect();
  ...
  isFieldMapped(field: SchemaTreeNode): boolean {
    if (this.side === 'source') {
  ...
  getFieldMappingCount(field: SchemaTreeNode): number {
    if (this.side === 'source') {
  ...
  hasDefaultValue(field: SchemaTreeNode): boolean {
    return this.defaultValues.some(d => d.targetField.id === field.id);
  }

  getDefaultValueDisplay(field: SchemaTreeNode): string {
  ...
  onFieldClick(event: MouseEvent, field: SchemaTreeNode): void {
  ...
  trackByFieldId(index: number, field: SchemaTreeNode): string {
    return field.id;
  }
}
