...
export class MappingService {
   ...
   */
  private stripFieldForStorage(field: SchemaTreeNode): SchemaTreeNode {
    const { children, expanded, ...rest } = field;
    return rest;
  }

  startDrag(field: SchemaTreeNode, startPoint: { x: number; y: number }): void {
    this.dragState.set({
      isDragging: true,
      sourceField: field,
      startPoint,
  ...
  changeSourceField(mappingId: string, newSourceField: SchemaTreeNode, sourceFieldIndex?: number): void {
    const mapping = this.mappings().find(m => m.id === mappingId);
    if (!mapping) return;
    ...
    if (sourceFieldIndex !== undefined && mapping.sourceFields.length > 1) {
      ...
      this.mappings.update(mappings =>
        mappings.map(m => m.id === mappingId ? { ...m, sourceFields: [strippedField] } : m)
      );
    }
  }

  changeTargetField(mappingId: string, newTargetField: SchemaTreeNode): void {
    const mapping = this.mappings().find(m => m.id === mappingId);
    if (!mapping) return;
    ...
    if (existingMapping) {
      ...
      this.mappings.update(mappings =>
        mappings.map(m => m.id === mappingId ? { ...m, targetField: this.stripFieldForStorage(newTargetField) } : m)
      );
    }
  }
  ...
  getArrayMappingForField(field: SchemaTreeNode): ArrayMapping | undefined {
  ...
  setDefaultValue(targetField: SchemaTreeNode, value: string | number | boolean | Date | null): DefaultValue {
